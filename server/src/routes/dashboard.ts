// Dashboard routes - fetches and caches health data from Oura
...
// Get date string in YYYY-MM-DD format, optionally offset by days
...
// Get today's health data - sleep, readiness, activity, heart rate, etc
router.get(
  "/today",
  requireAuth,
    ...
    try {
      ...
      if (!ouraToken) {
        return res
          ...
          .json({
            error:
              "No Oura token configured. Please add your Oura personal access token in settings.",
          });
      }
      ...
      const [
        ...
        fetchSleepPeriods(yesterday, today, ouraToken).catch(() => ({
          data: [],
        })),
      ]);
...
// Get 7-day historical data for trend charts
router.get(
  "/week",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const [sleepRange, readinessRange, activityRange, sleepPeriods] =
        await Promise.all([
          fetchDailySleepRange(weekAgo, today, ouraToken).catch(() => ({
            data: [],
          })),
          fetchDailyReadinessRange(weekAgo, today, ouraToken).catch(() => ({
            data: [],
          })),
          fetchDailyActivityRange(weekAgo, today, ouraToken).catch(() => ({
            data: [],
          })),
          fetchSleepPeriods(weekAgo, today, ouraToken).catch(() => ({
            data: [],
          })),
        ]);
...
// Clear cached data so next request fetches fresh from Oura
router.post(
  "/sync",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Clear all cached dashboard data for this user
      cache.clearUser(userId);

      res.json({
        success: true,
        message: "Cache cleared. Refresh to get fresh data.",
      });
    } catch (error) {
